...
TMP_FOLDER=$(mktemp -d)
NAME_COIN="HeraCoin"
GIT_REPO="https://github.com/Heraproject/HeraCoin.git"
BINARY_FILE="Herad"
...
function create_key() {
	  ...
	  KEY_COIN=$(sudo -u $WORKER $BINARY_PATH -conf=$WORKER_FOLDER/$CONFIG_FILE -datadir=$WORKER_FOLDER masternode genkey)
	  sudo -u $WORKER $BINARY_PATH -conf=$WORKER_FOLDER/$CONFIG_FILE -datadir=$WORKER_FOLDER stop
	  fi
}
...
masternodeaddr=$NODEIP:$PORT_COIN
